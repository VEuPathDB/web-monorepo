...
If you're working on the cedar development server, you need to address a PATH configuration issue that can shadow Volta's yarn with an older system version.
...
After making this change, start a new shell and run:
...
Initial operations on cedar are slower than on development laptops:

- `git clone` of this repository: ~10-15 minutes
- Initial `yarn install`: ~10-15 minutes

This is normal due to cedar's network and I/O configuration. Subsequent yarn operations are much faster.
