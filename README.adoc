...
For additional security, set this environment variable in your shell profile (`.bashrc`, `.zshrc`, etc.):
...
This requires NPM packages to be at least 7 days old (10080 minutes) before installation, protecting against supply chain attacks with newly published malicious packages.

*Note*: This is a Yarn 4+ feature and will be ignored by Yarn 3.x on legacy deployments.
...
[source, bash]
----
# Use bundled Yarn 3.3.1
export YARN_YARN_PATH=.yarn/releases/yarn-3.3.1.cjs
