...
[source, bash]
----
# Use corepack (Yarn 4.12.0) instead of bundled Yarn 3.3.1
export YARN_IGNORE_PATH=true

# Supply chain security: require packages to be 7+ days old
...
3. In this repository, create your `.envrc` file:
+
[source, shell]
----
...
# Do NOT set YARN_IGNORE_PATH - the bundled Yarn 3.3.1 should be used
----

The repository's `yarnPath` setting in `.yarnrc.yml` will automatically make `yarn` commands use the bundled Yarn 3.3.1, even when called from Ant or other build tools. No need to call Yarn with a fully-qualified path.

*CRITICAL*: The `yarn.lock` file uses Yarn 4 format. When Yarn 3 runs `yarn install`, it will modify checksums in the lockfile. These changes are format-only (no package version changes) but must *NEVER* be committed to git, as they will break Yarn 4 on development machines.
