...
🆕 In that terminal window you can now run
[source, shell]
```
yarn workspace @veupathdb/eda build-forever
```

**If the first build is successful**, the script will then watch for changes to the files and rebuild as necessary. The webpack dev server will automatically pick up the rebuilt dependencies and reload the page for you. Not as smooth as `gemomics-site` edits, but better than nothing!
