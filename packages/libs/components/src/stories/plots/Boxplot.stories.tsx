...
BelowZero.argTypes = storyArgTypes(
  (BelowZero.args = {
    boxplotData: [{ ...outdoorTemperatureData, label: 'outdoor temperature' }],
  })
);
...
function storyArgTypes(args: any): any {
  const pointTraceIndices = args.boxplotData
  ...
  return {
    showRawData: {
      control: {
        disable:
          args.boxplotData.filter((d: any) => d.rawData && d.rawData.length)
            .length == 0,
      },
    },
    showMean: {
      control: {
        disable:
          args.boxplotData.filter((d: any) => d.mean !== undefined).length == 0,
      },
    },
    markerOpacity: {
      control: {
