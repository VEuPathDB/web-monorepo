...
const classify = makeClassifier('DatasetManagement', 'BigwigDatasetDetail');
...
interface CompatibilityRow extends DatasetDependency {
  project: string;
}

interface BigwigManagementState extends DatasetManagementState {
...
class BigwigDatasetManagement extends DatasetManagement<BigwigManagementState> {
  ...
  getTracksTableColumns(): Array<MesaColumn<TracksRow>> {
  ...
  renderTracksSection() {
    ...
    const installFiles = userDataset.files?.install?.contents
  ...
  renderCompatibilitySection() {
    ...
    const compatibilityTableState = MesaState.create({
      columns: this.getCompatibilityTableColumns(),
  ...
  getCompatibilityTableColumns(): Array<MesaColumn<CompatibilityRow>> {
    const { userDataset } = this.props;
    const { installTargets: projects } = userDataset;
    return [
      {
        key: 'project',
  ...
  // See note in the base class, DatasetManagement
...
export default BigwigDatasetManagement;
