...
import { ActionThunk, EmptyAction, emptyAction } from '../Core/WdkMiddleware';
...
export const PROFILE_FORM_SUBMISSION_STATUS =
  'user/profile-form-submission-status';
...
export function profileFormSubmissionStatus(
  formStatus: FormStatus,
  formData: UserProfileFormData,
  errorMessage?: string
): ProfileFormSubmissionStatusAction {
...
export function passwordFormUpdate(payload: {
  oldPassword: string;
  newPassword: string;
  confirmPassword: string;
}): PasswordFormUpdateAction {
...
export const PASSWORD_FORM_SUBMISSION_STATUS =
  'user/password-form-submission-status';
...
export function passwordFormSubmissionStatus(
  formStatus: FormStatus,
  errorMessage?: string
): PasswordFormSubmissionStatusAction {
  return {
...
export function resetPasswordUpdateEmail(
  email: string
): ResetPasswordUpdateEmailAction {
  return {
...
export const RESET_PASSWORD_SUBMISSION_STATUS =
  'user/reset-password-submission-status';
...
export function resetPasswordSubmissionStatus(
  message?: string
): ResetPasswordSubmissionStatusAction {
  return {
...
export function basketStatusLoading(
  record: RecordInstance
): BasketStatusLoadingAction {
  return {
...
export function basketStatusReceived(
  record: RecordInstance,
  status: boolean
): BasketStatusReceivedAction {
  return {
...
export function basketStatusError(
  record: RecordInstance,
  error: Error
): BasketStatusErrorAction {
  return {
...
export function favoritesStatusLoading(
  record: RecordInstance
): FavoritesStatusLoadingAction {
  return {
...
export function favoritesStatusReceived(
  record: RecordInstance,
  id?: number
): FavoritesStatusReceivedAction {
  return {
...
export function favoritesStatusError(
  record: RecordInstance,
  error: Error
): FavoritesStatusErrorAction {
  return {
 ...
 */
export function conditionallyTransition(
  test: UserPredicate,
  path: string,
  external: boolean = false
): ActionThunk<EmptyAction | RouteAction> {
...
export function updateUserPreference(
  ...
  key: string,
  value: string
): ActionThunk<PreferenceUpdateAction> {
  return function run({ wdkService }) {
    let updatePromise = wdkService.patchSingleUserPreference(scope, key, value);
    return sendPrefUpdateOnCompletion(
      updatePromise,
      preferenceUpdate({ [scope]: { [key]: value } } as UserPreferences)
    );
  };
}
...
): ActionThunk<PreferencesUpdateAction> {
  return function run({ wdkService }) {
    let updatePromise = wdkService.patchUserPreferences(newPreferences);
    return sendPrefUpdateOnCompletion(
      updatePromise,
      preferencesUpdate(newPreferences)
    );
  };
}

function sendPrefUpdateOnCompletion<
  PrefAction extends PreferenceUpdateAction | PreferencesUpdateAction
>(promise: Promise<UserPreferences>, action: PrefAction): Promise<PrefAction> {
...
type SubmitProfileFormType = ActionThunk<
  UserUpdateAction | PreferencesUpdateAction | ProfileFormSubmissionStatusAction
>;
export function submitProfileForm(
  formData: UserProfileFormData
): SubmitProfileFormType {
  return function run({ wdkService }) {
    let partialUser: Partial<User> = <UserProfileFormData>(
      filterOutProps(formData, ['isGuest', 'id', 'confirmEmail', 'preferences'])
    );
    let userPromise = wdkService
      .getCurrentUser()
      .then((user) =>
        wdkService.updateCurrentUser({ ...user, ...partialUser })
      );
    return [
      ...
      userPromise
        .then((user) => [
          wdkService
            .patchUserPreferences(formData.preferences as UserPreferences)
            .then((prefPromise) => [
...
/** Register user */
type SubmitRegistrationFormType = ActionThunk<
  ProfileFormSubmissionStatusAction | ClearRegistrationFormAction
>;
export function submitRegistrationForm(
  formData: UserProfileFormData
): SubmitRegistrationFormType {
  return function run({ wdkService }) {
    let trimmedUser = <User>(
      filterOutProps(formData, ['isGuest', 'id', 'preferences', 'confirmEmail'])
    );
    ...
    return [
      profileFormSubmissionStatus('pending', formData),
      wdkService
        .createNewUser(registrationData)
        .then((responseData) => [
...
export function savePassword(
  oldPassword: string,
  newPassword: string
): ActionThunk<PasswordFormSubmissionStatusAction> {
  return function run({ wdkService }) {
    return [
...
export function submitPasswordReset(
  email: string
): ActionThunk<ResetPasswordSubmissionStatusAction> {
      ...
      wdkService.resetUserPassword(email).then(
        () => {
          ...
          transitioner.transitionToInternalPage(
            '/user/message/password-reset-successful'
          );
 ...
 */
function maybeLoggedIn<T extends ReduxAction>(
  loggedInThunk: ActionThunk<T>
): ActionThunk<T | EmptyAction>;
function maybeLoggedIn<T extends ReduxAction, S extends ReduxAction>(
  loggedInThunk: ActionThunk<T>,
  guestThunk: ActionThunk<S>
): ActionThunk<T | S>;
function maybeLoggedIn<T extends ReduxAction, S extends ReduxAction>(
  loggedInThunk: ActionThunk<T>,
  ...
  return ({ wdkService }) =>
    wdkService
      .getCurrentUser()
      .then((user) => (user.isGuest ? guestThunk : loggedInThunk));
}
 ...
 * @param {Record} record
 */
export function loadBasketStatus(
  record: RecordInstance,
  recordClassUrlSegment: string
): ActionThunk<BasketAction | EmptyAction> {
  return maybeLoggedIn<BasketAction>(async ({ wdkService }) =>
    setBasketStatus(
      record,
      wdkService
        .getBasketStatus(recordClassUrlSegment, [record])
 ...
 */
export function updateBasketStatus(
  record: RecordInstance,
  recordClassUrlSegment: string,
  status: boolean
): ActionThunk<BasketAction | ShowLoginModalAction | EmptyAction> {
...
let setBasketStatus = (
  record: RecordInstance,
  basketStatusPromise: Promise<boolean>
): ActionThunk<BasketAction> => {
  return function run() {
    return [
 ...
 * @param {Record} record
 */
export function loadFavoritesStatus(
  record: RecordInstance,
  recordClassUrlSegment: string
): ActionThunk<FavoriteAction | EmptyAction> {
  return maybeLoggedIn<FavoriteAction>(({ wdkService }) =>
    setFavoritesStatus(
      record,
      wdkService.getFavoriteId(record.id, recordClassUrlSegment)
    )
  );
}
...
export function removeFavorite(
  record: RecordInstance,
  favoriteId: number
): ActionThunk<FavoriteAction | ShowLoginModalAction | EmptyAction> {
  return maybeLoggedIn<FavoriteAction, ShowLoginModalAction | EmptyAction>(
    ({ wdkService }) =>
      setFavoritesStatus(record, wdkService.deleteFavorite(favoriteId)),
...
export function addFavorite(
  ...
  return maybeLoggedIn<FavoriteAction, ShowLoginModalAction | EmptyAction>(
    ({ wdkService }) =>
      setFavoritesStatus(
        record,
        wdkService.addFavorite(record.id, recordClassUrlSegment)
      ),
    () => showLoginWarning('use favorites')
  );
}
 ...
 */
function setFavoritesStatus(
  record: RecordInstance,
  statusPromise: Promise<number | undefined>
): ActionThunk<FavoriteAction> {
  return function run() {
    return [
