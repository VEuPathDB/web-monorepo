...
export function generateDayListByMonth(
  monthNumber = 1,
  year = currentYear
): number[] {
...
export function isValidDateString(dateString: string): boolean {
  ...
  const segments: number[] = dateString
    .split('-')
    .map((segment: string) => parseInt(segment));
...
export function isValidDateObject(dateObject: object): boolean {
  ...
  return (
    Object.entries(dateObject).filter((entry: any[]) => {
...
export function conformDayWithinMonth(
  day: number,
  month: number,
  year: number
): number {
...
export function conformDateToBounds(
  targetDate: DateObject,
  targetBounds: DateRangeObject
): DateObject {
  ...
  if (year === start.year && month === start.month && day < start.day)
    day = start.day;
  else if (year === end.year && month === end.month && day > end.day)
    day = end.day;
...
export function parseDate(dateString: string): DateObject {
  if (!isValidDateString(dateString))
    throw new Error('invalid date given to [parseDate()]: ' + dateString);
  let [year, month, day] = dateString
    .split('-')
    .map(function numParse(segment: string): number {
