...
export default function CardList(props) {
  ...
  React.useEffect(() => {
    ...
    if (isExpanded == null) {
      ...
      sessionStorage.setItem(
        isExpandedStorageKey,
        isExpanded ? 'true' : 'false'
      );
    }

    if (filterString == null) {
      const initialFilterString = sessionStorage.getItem(
        filterStringStorageKey
      );
  ...
  const className =
    `${CLASS_NAME} ${isLoading ? LOADING_CLASS_NAME : ''} ` +
    `${isEmpty ? EMPTY_CLASS_NAME : ''} ` +
    `${isExpandable ? EXPANDABLE_CLASS_NAME : ''} ` +
    `${isSearchable ? SEARCHABLE_CLASS_NAME : ''} ` +
    `${
      isExpandable && isExpanded ? EXPANDED_CLASS_NAME : ''
    } ${additionalClassName}`;

  const cardElements =
    list == null || isEmpty || isLoading
      ? Array(5)
          .fill(null)
  ...
  const expandButton = isExpandable && (
    <Tooltip title={buttonTitle}>
      ...
      >
        {isExpanded ? (
          <React.Fragment>
            <i className="fa fa-ellipsis-h" aria-hidden="true"></i> Row view
          </React.Fragment>
        ) : (
          <React.Fragment>
      ...
      options={filters.map((filter) => ({
        ...
        label: (
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ transform: 'scale(.75)' }}>{filter.display}</div>{' '}
            {upperFirst(filter.id)}
          </div>
        ),
      }))}
  ...
  const tableLink = tableViewLink && (
    <Tooltip title={'View content as a table'}>
      <Link to={tableViewLink} className={ALLSTUDIES_LINK_CLASS_NAME}>
        <i className="ebrc-icon-table" aria-hidden="true"></i>{' '}
        {tableViewLinkText}
      </Link>
    </Tooltip>
  );
