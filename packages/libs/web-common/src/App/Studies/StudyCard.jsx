...
class StudyCard extends React.Component {
  ...
  renderLinkouts() {
    ...
    if (useEda) {
        ...
        <div className="StudyCard-LinkOuts">
          ...
          {analyses?.some((analysis) => analysis.studyId === card.id) && (
            <div className="box StudyCard-MyAnalyses">
              <Link
                to={{
                  pathname: makeEdaRoute(),
                  search: `?s=${encodeURIComponent(card.name)}`,
                }}
              >
      ...
      return (
        <DownloadLink
          className="box StudyCard-Download"
        ...
        <Link className="StudyCard-SearchLink" to={edaRoute}>
          <div className="box StudyCard-PreFooter">
            {isPrereleaseStudy(card.access, card.id, permissions) ? (
              ...
              <span>
                {disabled ? 'Explore Unavailable' : 'Explore The Data'}
              </span>
            )}
          </div>
  ...
  render() {
    ...
    return (
      <div
        className={
          'Card StudyCard ' + (disabled ? 'disabled' : '') + ' StudyCard__' + id
        }
      >
        <div className="box StudyCard-Heading">
          <h2>
            <Tooltip title={myStudyTitle}>
              <Link to={useEda ? edaRoute + '/details' : route}>
                {safeHtml(name)}
              </Link>
            </Tooltip>
          </h2>
        ...
        <Tooltip title={myStudyTitle}>
          <Link
            to={useEda ? edaRoute + '/details' : route}
            className="StudyCard-DetailsLink"
          >
            <small>
              Study Details <Icon fa="chevron-circle-right" />
            </small>
          </Link>
        </Tooltip>
        <div className="box StudyCard-Stripe">{headline}</div>
