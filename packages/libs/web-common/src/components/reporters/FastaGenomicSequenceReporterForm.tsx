...
interface FastaGenomicSequenceFormState {
  revComp: boolean;
  start: number;
  end: number;
...
interface FastaGenomicSequenceFormUiState {}

interface FastaGenomicSequenceReporterFormProps {
  formState: FastaGenomicSequenceFormState;
  updateFormState: (state: FastaGenomicSequenceFormState) => void;
  onSubmit: () => void;
  includeSubmit: boolean;
}

export const fastaGenomicSequenceReporterFormFactory = (
  regionDescription: string
) => {
  const FastaGenomicSequenceReporterForm: React.FC<FastaGenomicSequenceReporterFormProps> & {
    getInitialState: () => {
      formState: FastaGenomicSequenceFormState;
      formUiState: FastaGenomicSequenceFormUiState;
    };
  } = (props) => {
    ...
    return (
      <div>
        ...
        <div style={{ margin: '2em' }}>
          <Checkbox
            name="revComp"
            value={formState.revComp}
            onChange={getUpdateHandler('revComp')}
          />{' '}
          Reverse & Complement
        </div>
        <div
        ...
        >
          ...
          <TextBox
            ...
            onChange={getUpdateHandler('start')}
            size="6"
          />{' '}
          to
          <TextBox
        ...
        <div>
          ...
          <ul>
            <li>
              ...
              to retrieve the complete sequence for the requested genomic
              regions, use "Nucleotide positions 1 to 10000"
            </li>
            <li>
              <i>
                <b>specific sequence region</b>
              </i>{' '}
              to retrieve a specific region for the requested genomic regions,
              use "Nucleotide positions
