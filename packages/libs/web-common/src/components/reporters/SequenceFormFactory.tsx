...
interface SequenceOptionsProps<T> {
  formState: T & {
    deflineType: string;
    deflineFields: string[];
...
const sequenceOptions = <T,>(props: SequenceOptionsProps<T>) => {
...
interface SequenceFormProps<T> {
  formState: T;
  updateFormState: (state: T) => void;
  onSubmit: () => void;
  includeSubmit: boolean;
  viewFilters?: FilterValueArray;
  updateViewFilters: (filters: FilterValueArray) => void;
  resultType: ResultType;
}

const createSequenceForm = <T extends Record<string, any>>(
  formBeforeCommonOptions: React.FC<SequenceFormProps<T>>,
  formAfterSubmitButton: React.FC<SequenceFormProps<T>>,
  getFormInitialState: () => Partial<T>,
  reportType: string
) => {
  const Form: React.FC<SequenceFormProps<T>> & {
    getInitialState: () => {
      formState: T & {
