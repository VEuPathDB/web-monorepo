...
interface SequenceGeneFormState {
  type: string;
  reverseAndComplement: boolean;
  ...
  endOffset3: number;
  dnaComponent: string;
  transcriptComponent: string;
  proteinFeature: string;
  splicedGenomic: string;
...
interface SequenceRegionRangeProps {
  ...
  sign: string;
  offset: string;
  formState: SequenceGeneFormState;
...
const SequenceRegionRange: React.FC<SequenceRegionRangeProps> = (props) => {
  ...
  return (
    <React.Fragment>
      ...
      <NumberSelector
        name={offset}
        value={formState[offset] as number}
        start={0}
        end={10000}
        step={1}
...
interface ProteinSequenceRegionInputsProps {
  formState: SequenceGeneFormState;
  getUpdateHandler: (fieldName: string) => (value: any) => void;
}

const ProteinSequenceRegionInputs: React.FC<ProteinSequenceRegionInputsProps> =
  (props) => {
    const { formState, getUpdateHandler } = props;
    return (
      <div>
...
interface FormBeforeCommonOptionsProps {
  formState: SequenceGeneFormState;
  updateFormState: (state: SequenceGeneFormState) => void;
  viewFilters?: FilterValueArray;
}

const formBeforeCommonOptions: React.FC<FormBeforeCommonOptionsProps> = (
  props
) => {
...
const formAfterSubmitButton: React.FC<any> = (props) => {
  return (
    <React.Fragment>
      <div>
        <hr />
...
const getFormInitialState = (): Partial<SequenceGeneFormState> => ({
  type: 'genomic',
