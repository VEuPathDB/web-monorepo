...
function OrganismAnswerController(
  props: Props & { DefaultComponent: ComponentType<Props> }
) {
  const organismAnswerProps = useExternallyFilteredAnswerProps(
    props,
    makeOrganismFilterPredicate
  );

  return <props.DefaultComponent {...organismAnswerProps} />;
}

function DatasetAnswerController(
  props: Props & { DefaultComponent: ComponentType<Props> }
) {
  const datasetAnswerProps = useExternallyFilteredAnswerProps(
    props,
    makeDatasetFilterPredicate
  );
...
function useExternallyFilteredAnswerProps(
  ...
  const { visibleRecords, totalCount } = useMemo(() => {
    ...
    return {
      visibleRecords: makeVisibleRecords(
...
function makeVisibleRecords(
  records: RecordInstance[] | undefined,
  filterPredicate: RecordFilterPredicate
) {
...
function makeTotalCount(
  ...
  return (
    records?.reduce(
      (count, record) => count + (filterPredicate(record) ? 1 : 0),
      0
    ) ?? 0
  );
}

function makeOrganismFilterPredicate(
  preferredOrganismsEnabled: boolean,
  preferredOrganisms: Set<string>
): RecordFilterPredicate {
  return (record) =>
...
function makeDatasetFilterPredicate(
  preferredOrganismsEnabled: boolean,
  preferredOrganisms: Set<string>
): RecordFilterPredicate {
  return (record) =>
