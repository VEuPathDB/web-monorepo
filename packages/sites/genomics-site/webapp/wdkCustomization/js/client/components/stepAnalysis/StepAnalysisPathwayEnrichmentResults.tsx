import {
  ...
  decimalCellFactory,
  integerCell,
} from '@veupathdb/wdk-client/lib/Components/StepAnalysis/Utils/StepAnalysisResults';
...
const pathwayIdRenderFactory =
  ...
  ({ row }: Record<string, any>) =>
    (
      <a
        href={`${pathwayBaseUrl}${row.pathwaySource}/${row.pathwayId}`}
        target="_blank"
      >
        {row.pathwayId}
      </a>
  ...
  ({ analysisResult, analysisConfig, webAppUrl }) => {
    ...
    return (
      <Fragment>
        ...
        <CommonResultTable
          emptyResultMessage={
            'No enrichment was found with significance at the P-value threshold you specified.'
          }
          ...
          columns={pathwayEnrichmentResultColumns.map((column) =>
            column.key === 'pathwayId'
              ? {
                  ...column,
                  renderCell: pathwayIdRenderFactory(
                    analysisResult.pathwayBaseUrl
                  ),
                }
              : column
          )}
        ...
        />
        <WordCloudModal
          imgUrl={`${webAppUrl}/service/users/current/steps/${
            analysisConfig.stepId
          }/analyses/${analysisConfig.analysisId}/resources?path=${
            analysisResult.imageDownloadPath
          }&_=${Date.now()}`}
          open={wordCloudOpen}
          onClose={() => {
