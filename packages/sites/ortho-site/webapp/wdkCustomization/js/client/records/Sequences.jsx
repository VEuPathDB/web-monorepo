...
export function RecordTable_Sequences(props) {
  ...
  const sortedValue = useSortedValue(props.value);
    ...
    return (
      <form ref={formRef} action="/cgi-bin/msaOrthoMCL" target="_blank" method="post">
        <input type="hidden" name="project_id" value="OrthoMCL"/>

        <props.DefaultComponent {...props} value={sortedValue}/>
        <input type="button" name="CheckAll" value="Check All" onClick={() => toggleAll(true)}/> 
        <input type="button" name="UnCheckAll" value="Uncheck All" onClick={() => toggleAll(false)}/>
        <br/>
